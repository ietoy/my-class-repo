...
// declare a variable that is a string of all 26 lowercase letters in the english alphabet we can use a string in this situation because we're only checking against individual characters if we were to have to validate non-alphanumeric characters, we would have to use an array
...
document.onkeyup = function(input) {
  ...
  if (alphabet.includes(userKey)) {
    // if the key is part of the alphabet, set the text value of div on our webpage we're using to display thecurrent key to that key
  ...
  // if the key is not part of our alphabet, change the current key div to ask the user to press a key that's a letter within our alphabet
    document.getElementById("user-key").textContent = "Please press a key that is a letter within our alphabet.";
  }
}
