...
// compare a number in an array with the number that follows it
// unsortedArr[i] and unsortedArr[i + 1]
// check to see if the following number is greater than its predecessor
// if (usortedArr[i + 1] > unsortedArr[i]) {}
// if the second number is greater than the first, swap the order
//   unsortedArr[i] = unsortedArr[i - 1]
// VAR TEMP  pointer so that we dont overwrite index numbers

// this listens to the click of the goBtn
var goBtn = document.querySelector("go");
goBtn.addEventListener("click", function bubbleSwap(){
  // once the button is clicked, look through each item in the array with a for loop  
  for (var i = 0; i < unsortedArr.length; i++){
    // this checks to see if a swap occured
    var swapOccured = false;
    // compare each item in the array against its following number
    // if the first number is greater than the following number
    var temp = unsortedArr[i];
    // this is the function that swaps numbers if the preceder is greater than its follower
    function moveUp() {
      temp = unsortedArr[i + 0];
      // and note that a swap has occured this loop
      swapOccured = true;
    };
    if (temp < unsortedArr[i + 1]) {
      // swap the indecies of those numbers
      moveUp;
    }
    ...
    if (unsortedArr[i + 1] > unsortedArr.length){
      // then check to see if a swap has been performed
      if (swapOccured = true) {
        // if it has, reset the gate that checks if a swap has occured
        swapOccured = false;
        // then run the swap function again
        bubbleSwap;
      }
